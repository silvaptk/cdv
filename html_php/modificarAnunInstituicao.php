<?php
    ...
    else
    {
		...
		$modificou = false;
		if(!empty(trim($titulo)) || !empty(trim($descricao)) || !empty(trim($categoria)))
		{
			$modificou = true;
			$atualizar = $bd->query("CALL sp_modAnunInstituicao($anuncio, '$titulo', '$descricao', '$categoria')") or die ("Erro no UPDATE da instituição - " . mysqli_error($bd));

				if($atualizar)	
				{				
                    $_SESSION["tipoMensagem"] = "alterarAnuncio";
                    $_SESSION["mensagemErro"] = "Alteração realizada com sucesso!";
					header("Location:perfilInstituicao.php");
				}
				else
				{
					$_SESSION["mensagemErro"] = "Erro ao atualizar os dados.";
					header("Location:modificarAnuncioInstituicao.php");
				}
		}
